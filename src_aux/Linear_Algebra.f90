...
real*8, intent(in)  :: Matrix(:,:), Constants(:)
real*8, intent(out) :: Variables(:)

!integer(4) :: Number_Equations, Pivoting(ubound(Matrix,1)), info
!real(8)    :: Array(ubound(Matrix,1),ubound(Matrix,2))
...
real*8    :: C(ubound(Matrix,1),ubound(Matrix,2)+1)
